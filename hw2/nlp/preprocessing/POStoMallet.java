...
public class POStoMallet {
	...
	for (int i = 0; i < files.length; i++) {
	  ...
	  if (!file.isDirectory()) {
		if (!file.getName().contains("CHANGES.LOG"))
		  sentences.addAll(new POStoMallet(file).tokenLists());
	  }
	  else 
	  {
  ...
  public static void writeToMallet(File[] files, File ofile) {
